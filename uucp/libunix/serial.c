...
#if USE_RCS_ID
const char serial_rcsid[] = "$FreeBSD: src/gnu/libexec/uucp/libunix/serial.c,v 1.11 1999/08/27 23:33:09 peter Exp $";
#endif
	  ...
	  if (q->fterminal)
	    {
	      ...
	      if (cwait > cread)
		cwait = cread;
	      stime.tv_sec = (cwait * 10) / (q->ibaud? q->ibaud: (long)1200);
	      stime.tv_usec = ((((cwait * 1000000) / (q->ibaud? q->ibaud: (long)1200)) * 10)
			       % 1000000);
	    }
	  else
	    {
